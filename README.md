...
> >
> 限于作者本人的技术水平，有些问题可能考虑的并不周全，若对该框架的 `UI` 或功能组件的使用有更好的建议，亦或有更多的定制化需求，组件存在 `BUG` 等，欢迎来 [这里](https://github.com/lirongtong/miitvip-vue-admin-manager/issues) 提 `issues`，我将尽可能的抽时间去解决相应的 `BUG`，去完善合理的定制化需求。
> >
> `Makeit Admin Pro 1.x` 版本属于尝鲜阶段的版本，`Makeit Admin Pro 2.x` 版本将迎来重大更新，解决 `1.x` 版本中存在的一些问题，`按需加载` 的功能实现重新设计，更多的定制化组件，同时将结合 `API`，形成一套完整的管理系统（包含 `菜单管理`，`部门管理`，`权限管理`，`地域选择` 等非常常用的后台管理系统功能模块）示例站点，拿来即用，为的就是让中后台管理系统的构建发布更加快速方便。`2.x` 版本也将借鉴阿里乾坤框架，对 `微前端` 的架构方面进行力所能及的适配。
