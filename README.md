...
> >
> `Makeit Admin Pro 2.x` 版本将迎来重大更新，解决 `1.x` 版本中存在的一些问题，`按需加载` 的功能实现重新设计，更多的定制化组件等，同时将结合 `API`，形成一套完整的管理系统（包含 `菜单管理`，`部门管理`，`权限管理` 等非常常用的后台管理系统功能模块），拿来即用。`2.x` 版本也将对 `微前端` 架构方面进行力所能及的适配，只为了开发人员在使用 `Makeit Admin Pro` 独立构建、部署 `微应用` 的同时，能更加方便的将 `微应用` 集成至 `主应用` 内。
